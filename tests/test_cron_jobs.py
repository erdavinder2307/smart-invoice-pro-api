...
class TestCheckLowStock:
    ...
    def test_low_stock_detected(self, mock_gc, mock_notif, client, cron_headers):
        ...
        resp = client.get("/api/cron/check-low-stock?send_email=false", headers=cron_headers)
    ...
    @patch("smart_invoice_pro.api.cron_jobs.create_notification")
    @patch("smart_invoice_pro.api.cron_jobs.get_container")
    def test_no_low_stock(self, mock_gc, mock_notif, client, cron_headers):
        products_ctr = MagicMock()
        stock_ctr = MagicMock()
    ...
    @patch("smart_invoice_pro.api.cron_jobs.create_notification")
    @patch("smart_invoice_pro.api.cron_jobs.get_container")
    def test_stock_calculation_in_out(self, mock_gc, mock_notif, client, cron_headers):
    ...
    @patch("smart_invoice_pro.api.cron_jobs.create_notification")
    @patch("smart_invoice_pro.api.cron_jobs.get_container")
    def test_reorder_level_zero_skipped(self, mock_gc, mock_notif, client, cron_headers):
    ...
    @patch("smart_invoice_pro.api.cron_jobs.create_notification")
    @patch("smart_invoice_pro.api.cron_jobs.get_container")
    def test_email_sent_when_enabled(self, mock_gc, mock_notif, mock_email, client, cron_headers):
        products_ctr = MagicMock()
        stock_ctr = MagicMock()
        products_ctr.read_all_items.return_value = [PRODUCT_A]
        stock_ctr.read_all_items.return_value = []
        ...
        resp = client.get("/api/cron/check-low-stock?send_email=true", headers=cron_headers)
        assert resp.status_code == 200
    ...
    @patch("smart_invoice_pro.api.cron_jobs.create_notification")
    @patch("smart_invoice_pro.api.cron_jobs.get_container")
    def test_post_method_also_works(self, mock_gc, mock_notif, client, cron_headers):
        products_ctr = MagicMock()
        stock_ctr = MagicMock()
        ...
        resp = client.post("/api/cron/check-low-stock?send_email=false", headers=cron_headers)
        assert resp.status_code == 200

    @patch("smart_invoice_pro.api.cron_jobs.create_notification")
    @patch("smart_invoice_pro.api.cron_jobs.get_container")
    def test_product_without_tenant_no_notification(self, mock_gc, mock_notif, client, cron_headers):
...
class TestScheduleInfo:
    ...
    def test_returns_schedule(self, client, cron_headers):
        resp = client.get("/api/cron/schedule-info", headers=cron_headers)
        assert resp.status_code == 200
        data = resp.get_json()
        assert "jobs" in data
...
class TestGenerateRecurringInvoices:
    ...
    def test_generates_invoice_for_due_profile(
        self, mock_rp, mock_inv, mock_audit, mock_notif, client, cron_headers
    ):
        ...
        with patch("smart_invoice_pro.api.recurring_profiles_api.calculate_next_run_date", return_value="2024-02-01"), \
            ...
            resp = client.post("/api/cron/generate-recurring", headers=cron_headers)
    ...
    @patch("smart_invoice_pro.api.cron_jobs.invoices_container")
    @patch("smart_invoice_pro.api.cron_jobs.recurring_profiles_container")
    def test_skips_profile_already_at_limit(self, mock_rp, mock_inv, client, cron_headers):
    ...
    @patch("smart_invoice_pro.api.cron_jobs.invoices_container")
    @patch("smart_invoice_pro.api.cron_jobs.recurring_profiles_container")
    def test_skips_profile_past_end_date(self, mock_rp, mock_inv, client, cron_headers):
    ...
    @patch("smart_invoice_pro.api.cron_jobs.recurring_profiles_container")
    def test_no_due_profiles_returns_zero(self, mock_rp, client, cron_headers):
